...
- `+1.0` For correctly matching a redacted sensitive token.
- `-1.0` For missing a sensitive token.
- `-0.5` Over-reduction (stripping safe textual utility).
- `+0.2` Rewarded for maintaining readability safely.
...
- **Hard:** Heavy token obfuscation contexts (e.g. `john at gmail dot com`, `five five five zero`).
...
docker run -p 7860:7860 privacy_env
```
